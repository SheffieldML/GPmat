...
%	Description:
%
%	SCG2 This is a slight modification of the netlab function for scg
%	(from Ian T Nabney) so that it can (optionally) receive an
%	ObjectiveGradient function (which returns both, the scalar value for
%	the objective function and the vector of partial derivatives) for
%	the first argument 'f', instead of an objective function. This means
%	that, when possible, the precomputations will be made only once and
%	used for both, the objective and the gradient function. However, the
%	gradient function must also be provided, because in a certain step
%	of the algorithm scg is interested in just finding the
%	gradient(whereas every calculation of the objective is always
%	accompanyied by a calculation of the gradient). This function can
%	work as the traditional scg if 'f' is a function with only one
%	output argument, otherwise, (if 'f' is a function with more than one
%	outputs) the behaviour described above will take effect. This is
%	done, therefore, only in the places of the original scg where f and
%	gradf are evaluated with the same set of parameters x (before they
%	change in the next optimisation cycle). Details:
