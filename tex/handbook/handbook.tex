...
\begin{equation} 
G(\tfConcentration_1(t),\ldots,\tfConcentration_1(t); \bfw_j, w_{j0})
...
and then it performs target identification in a genome-wide scale. 
...
\begin{tabular}{ccc}
      ...
      \includegraphics[width=0.3\textwidth]{./toyMarch25MCMC19-Sep-2010logOnePlusExpgenHillReplica2GENEmRNA10} \\
      (d) & (e) & (f)
 \end{tabular} 
\end{center}
...
\end{figure}
 ...
 \begin{figure}
  \begin{center}
      \includegraphics[width=0.8\textwidth]{./dros_toy_evaluation.pdf}
\end{center}
...
the first TF, while the right panel shows performance for predicting the second TF. 
Red curves show the results by using the multiple TF model
and blue lines the results by using the linear single-TF model. 
In both plots, random prediction is represented by the diagonal black
line.}\label{fig:toyMultipleTFsROCs}
\end{figure}
...
links between the two TFs and the $980$ test genes.  The performance of the
a linear single-TF model is also displayed and is clearly much inferior than
