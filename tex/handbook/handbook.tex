...
single input transcription factor, this is known as a single input
...
concentration\footnote{This would be equivalent to assuming very high
  decay and sensitivity the simple model we describe. Models like this
  ...
  some genomewide analysis models, for example
  \citep{Sanguinetti:chipvar06,Sanguinetti:chipdyno06}).} this is
...
\[
\mrnaConcentration_j(t) = a_je^{-\decayRate_{j}t} + \frac{\basalRate_j}{\decayRate_j} + \sensitivity_je^{-\decayRate_{j}t}\int_0^t e^{\decayRate_{j}u}\tfConcentration(u) \mathrm{d}u 
\]
...
considered single input motifs, dealt with the missing function
...
\begin{figure}
  ...
  \caption{The basis in \reffig{fig:basisFunctions} have been pushed
...
Given a basis set, $\left\{\phi_k(t)\right\}_{k=1}^\numBasisFunc$, a set of weights, $\left\{\weightScalar_k\right\}_{k=1}^\numBasisFunc$, and the parameters of the differential equation, $\{\basalRate_j, \decayRate_j, \sensitivity_j\}_{j=1}^\dataDim$, we can solve the differential equations for the mRNA concentrations, $\left\{\mrnaConcentration_j(t)\right\}_{j=1}^\dataDim$. We can determine all these parameters of the model (including the parameters of the basis functions) by fitting through maximum likelihood. If we assume Gaussian noise of variance $\dataStd^2$ we can compute the log likelihood of an observed data set of mRNA concentrations, perhaps obtained from gene expression microarray,
\[
...
interval, so we take $\gamma = \alpha_p\Delta\tau$. Now we have basis
...
\[
\kernel_{0,0}\left(t,t^\prime\right) = \frac{\alpha_p}{\pi \lengthScale^2}
...
\frac{\alpha_p}{\sqrt{2\pi\lengthScale{}^2}} \exp\left(
...
\cite{Scholkopf:learning01} due to the fact that the covariance
function has the properties of a Mercer kernel. A function of two variables can bee seen as a Mercer kernel if when a
symmetric matrix of values from $\kernel_{0,0}(t, t^\prime)$ is computed
for a vector of times $\mathbf{t}$ it should always be positive
...
\]
An observation of the TF may be in the form of a constraint that the TF concentration is known to be zero at time zero: $\tfConcentration(0)=0, \quad \dataStd^2_0=0$.  Or in some cases it could be a direct measurement. This gives us the full data set which we place in a vector stacked vector storing all the values,
\[
...
\begin{align*}
E\left(\parameterVector\right)& = -\frac{1}{2}\log p(\dataVector | \parameterVector)\\ & = \frac{1}{2}\log \det{\kernelMatrix}  + \frac{1}{2}\dataVector^\top\kernelMatrix^{-1}\dataVector + \mathrm{const.}
\end{align*}
with respect to the parameters $\parameterVector =
\left\{\left\{\sensitivity_j, \decayRate_j\right\}, \alpha_p,
...
\[
\diff{E\left(\parameterVector\right)}{\kernelMatrix} = -\frac{1}{2}\kernelMatrix^{-1}  + \frac{1}{2}\kernelMatrix^{-1} \dataVector\dataVector^\top\kernelMatrix^{-1},
\]
...
of scaled conjugate gradients \cite{Moller:scg93}, but other
