...
to be estimated. We develop exact (for linear regulation) and
approximate (for non-linear regulation) inference
...
A challenging problem for parameter estimation in ODE models occurs where
...
\caption{Results for p53 using the linear model and a GP with RBF
  ...
  inference, dashed lines show the 95\% credible intervals, and the
  crosses are the observed gene expression data with error bars
  showing the technical error from each individual Affymetrix
  microarray obtained using the puma package~\citep{Liu05}. Maximum likelihood model parameters,
  estimated using all replicates, are shown for each target
  gene. Data and reconstructed profiles are shown on an unlogged
  normalised scale. Time is measured in hours. } \label{fig:activation}
\end{figure*}
...
covariance function, we can analytically solve the integrals in
equations~(\ref{eqn:kxjxj}--\ref{eqn:kxixj}) and thereby define a probabilistic model
...
Once the model parameters have been estimated then the model can be
used to estimated the predictive GP distribution for $f(t)$ and for
each target gene $x_j(t)$. The mean
and covariance of this predictive distribution for $f(t)$ are given by,
\begin{eqnarray*}
\langle f \rangle_{\tiny\mbox{post}} & = & K_{f\bm x}K^{-1}_{\bm x\bm
  x}\bm x \\
K_{ff}^{\mbox{\tiny post}} & = & K_{ff} - K_{f\bm x}K^{-1}_{\bm x\bm
  x}K_{\bm x f} 
 \end{eqnarray*}
where upper case $K$ denotes a matrix with the following elements:
$(K_{ff})_{p,q} = k_{ff}(t_p,t_q)$, $(K_{f\bm
  x})_{p,kj}=k_{f,x_j}(t_p,t_k)$ and $(K_{\bm x\bm
  x})_{ki,lj}=k_{x_i,x_j}(t_l,t_k) + \delta_{ij}\delta_{lk}\sigma_{ik}^2$ where
$\sigma_{ik}^2$ is the measurement noise associated with
$x_i(t_k)$. Here $t_p$ and $t_q$ denote any choice of points over
which one wishes to evaluate the GP, whereas $t_k$ and $t_l$ are
specifically the times at which the data are measured. This equation was used to plot
the crebible intervals for $f$ in the figures presented in this
paper. It is similarly possible to compute the predictive distribution
for the data by replacing $f$ with $x_i$ in the above expression. The
predictive distribution can be used for ranking genes according to
their fit to the model (as in the applicartion considered by \citet{Barenco:ranked06}).
...
consistent since the MAP-Laplace approach is unconstrained in the choice of
...
\caption{Results for the repressor LexA. (a) predicted LexA
  ...
  microarray data set and therefore a gene-specific noise variance
  parameter was estimated for each target gene. Data and reconstructed
  profiles are given in the original unlogged scale. Time is measured in minutes.}
...
transcription is that there are a limited number
...
and we are using a GP $f\left(t\right)$ to model the log of the TF
activity.
...
non-linear response models and layered systems. \citet{Barenco:ranked06} have already shown that these
