...
simXrbfKernCompute <- function (simKern, rbfKern, t1, t2=t1) {
  ...
  options(warn=-1)
  if ( length(I1) > 0 )
    lnPart1[I1] <- expTransform(erfc(x1[I1]) - erfc(x2[I1]), "xtoa")
  if ( length(I2) > 0 )
...
simKernDiagCompute <- function (kern, t) {
  ...
  h <- exp(halfSigmaD*halfSigmaD)*(erf(-halfSigmaD)+erf(t/sigma+halfSigmaD))-exp(halfSigmaD*halfSigmaD-(2*kern$decay*t))*(erf(t/sigma-halfSigmaD)+erf(halfSigmaD))
