...
gpsimMapCreate <- function (numGenes, numProteins, times, y, yvar, options) {
  ...
  model <- list(type="gpsimMap", y=as.array(y), yvar=as.array(yvar), t=times, nonLinearity=options$nonLinearity, Transform="exp", B=options$B, D=options$D, S=options$S, optimiser=options$optimiser, gParam=options$gParam)
  ...
  if ( any(grep("gParam", names(model))) ) {
  ...
  for ( j in seq(length=model$numGenes) ) {
    ...
    for ( i in seq(length=model$numMapPts) ) {
      ...
      if ( option==1 ) {
        ...
        model$ypred[i,j] <- base + model$B[j]/model$D[j]+sum(model$g[start:i, gInd]*exp(log(model$S[j])-model$D[j]*tfs+lnintegral[start:i]))        
        ...
        for ( i in seq(length=numData) ) {
          ...
          if ( arg1>=0 & arg2>=0 )
            for ( j in seq(length=model$numGenes) ) {
              ...
              ind <- i + (j-1)*numData
              betaij <- 1/yvar[ind]
              temp <- temp+betaij*model$gGrad[k,gInd]*model$gGrad[l,gInd]*exp(-model$D[j]*(arg1+arg2)+log(S2[j])+log(step2))
            }
        }
        model$W[k,l] <- temp
      ...
      for ( i in seq(length=numData) ) {
        ...
        if ( arg>=0 )
          for ( j in seq(length=model$numGenes) ) {
            ...
            temp <- temp + factor*model$gGrad2[k,gInd]*exp(-model$D[j]*arg+log(model$S[j])+log(model$step))
          }
      }
    ...
    for ( i in seq(length=numData) ) {
      ...
      if ( arg>=0 )
        for ( j in seq(length=model$numGenes) ) {
          ...
          betaij <- 1/yvar[ind]
          factor <- (model$ypred[model$timesIndex[i], j]-model$y[ind])*betaij
          temp <- temp+factor*model$gGrad[k,gInd]*exp(-model$D[j]*arg)*model$S[j]
        }
    }
  ...
  if ( option == 1 ) {
    for ( m in seq(length=endPoint) ) {
      ...
      if ( arg>=0 ) {
        ...
        dxdS <- dxdS + exp(-model$D[j]*arg+log(model$step))*model$g[m,gInd]
        if ( model$ngParam > 0 ) 
          for ( gParamInd in seq(length=ngParamk) ) 
            dxdgParam[gParamInd] <- dxdgParam[gParamInd] + exp(-model$D[j]*arg+log(model$step)+log(model$S[j]))*model$dg[m,gInd]
      }
    }
    ...
    dxdS <- exp(-model$D[j]*model$t[i]+log(model$step))/3*sum(model$g[1:endPoint,gInd]*integral1[1:endPoint])
    
    if ( model$ngParam > 0 )
      for ( gParamInd in seq(length=ngParamk) )
        dxdgParam[gParamInd] <- exp(log(model$S[j])-model$D[j]*model$t[i]+log(model$step))/3*sum(model$dg[1:endPoint, gInd]*integral1[1:endPoint])
  }
  ...
  for ( p in intPoints )
    for ( i in seq(length=numData) ) {
      ...
      if ( arg >= 0 ) {
        ...
        dWdB[p, p] <- dWdB[p, p]+betaik*model$gGrad2[p,gInd]*dxdB*exp(-model$D[k]*arg+log(model$S[k]) +log(model$step))

        if ( any(grep("isGroupNonlinearity", names(model))) )
          if ( model$nonLinearity[k]=="repression" )
            dWdalpha[p,p] <- dWdalpha[p, p]+betaik*model$gGrad2[p,gInd]*dxdalpha*exp(-model$D[k]*arg+log(model$S[k]) +log(model$step))
                 
        factor <- model$ypred[model$timesIndex[i], k]-model$y[ind]

        dWdD[p, p] <- dWdD[p, p]+model$step*betaik*model$gGrad2[p,gInd]*(dxdD-factor*arg)*exp(-model$D[k]*arg+log(model$S[k]))

        dWdS[p, p] <- dWdS[p, p]+model$step*betaik*exp(-model$D[k]*arg)*(dxdS*model$S[k]*model$gGrad2[p,gInd]+factor*model$gGrad2[p,gInd])
        ...
        if ( model$includeNoise )
          dWdn[p, p] <- dWdn[p, p]-2*sqrt(model$noiseVar[k])*betaik^2*factor*model$gGrad2[p,gInd]*exp(-model$D[k]*arg+log(model$step)+log(model$S[k]))
      }
    }
    ...
    for ( q in intPoints )
      for ( i in seq(length=numData) ) {
        ...
        if ( arg1 >= 0 & arg2 >= 0 ) {
          ...
          dWdD[p,q] <- dWdD[p,q]-betaik*model$gGrad[p,gInd]*model$gGrad[q,gInd]*(arg1+arg2)*exp(-model$D[k]*(arg1+arg2)+log(S2[k])+log(step2))

          dWdS[p,q] <- dWdS[p,q]+2*betaik*model$S[k]*model$gGrad[q,gInd]*model$gGrad[p,gInd]*exp(-model$D[k]*(arg1+arg2)+log(step2))
          ...
          if ( model$includeNoise )
            dWdn[p, q] <- dWdn[p, q]-2*sqrt(model$noiseVar[k])*betaik^2*model$gGrad[p,gInd]*model$gGrad[q,gInd]*exp(-model$D[k]*(arg1+arg2)+log(step2)+log(S2[k]))
        }
      }
...
gpsimMapInitParam <- function (y, yvar, options, display=TRUE, sopt=FALSE) {
  ...
  initOptions <- options     
  mu <- apply(y, 2, mean)
  optionDset <- c(0.01, 0.1, 0.4, 1, 10)
  set.seed(10)
