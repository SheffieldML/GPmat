gpUpdateAD <- function (model, X) {
  ...
  if (model$approx == "ftc") {
	...
	for (i in 1:model$d) {
	  ...
	  model$innerProducts[1, i] = model$m[ind, i]%*%model$invK_uu[[i]]%*%model$m[ind, i]
	}
      }
    }
    ...
    if ((!"isSpherical" %in% names(model)) || model$isSpherical) {
	  ...
	  - model$beta%*%colSums(model$K_uf * (model$invK_uu%*%model$K_uf))
	...
	model$innerProducts[1, i] = model$beta%*%
	    (Dinvm%*%model$m[, i] - K_ufDinvm%*%model$Ainv%*%K_ufDinvm)
      }
	...
	model$diagD[[i]] = 1 + model$beta%*%model$diagK[ind]
	  - model$beta%*%colSums(model$K_uf[, ind]*(model$invK_uu%*%model$K_uf[, ind]))
	...
	K_ufDinvm = K_ufDinvm + model$K_uf[, ind]%*%Dinvm[[i]]
      ...
      model$innerProducts = matrix(0, 1, model$d)
      for (i in 1:model$d)
	model$innerProducts[1, i] = - model$beta%*%K_ufDinvm[, i]%*%model$Ainv%*%K_ufDinvm[, i]
	...
	model$logDetA[j] = 2* sum( log ( diag(invA$chol) ) )
      ...
      ## compute inner products
      for (j in 1:model$d)
	model$innerProducts[1, j] = - model$beta%*%K_ufDinvm[, j]%*%model$Ainv[[j]]%*%K_ufDinvm[, j]

      for (i in 1:length(model$blockEnd)) {
	for (j in 1:model$d) {
	  ind = gpDataIndices(model, j, i)
	  model$innerProducts[1, j] = model$innerProducts[1, j] + model$beta%*%Dinvm[[i]][ind, j]%*%model$m[ind, j]
	}
      }
    }
  } else
