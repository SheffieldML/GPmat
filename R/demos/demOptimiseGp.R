demOptimiseGp <- function(filename='demOptimiseGp') {
...
#     S = S - exp(2*inithypers[3]) ## subtract noise variance
    ...
    dev.set(2)
    gpPlot(model, xtest, mu, S, ylim=c(-2.5,2.5), xlim=range(xtest), col='black')
    counter = counter + 1
    dev.copy2eps(file =
      paste(path,'demOptimiseGp_1_', as.character(counter), '.eps', sep='') ) ## Save plot as eps
    ## Convert to png. Needs the 'eps2png' facility. If not already installed: 'sudo apt-get install eps2png'
    system(paste('eps2png ',path,'demOptimiseGp_1_',as.character(counter),'.eps',sep=''))

    dev.set(3)
    matplot(lengthScale[1:i], cbind(ll[1:i], llLogDet[1:i],  llFit[1:i]), type="l", lty=1, log="x",
      xlab='length-scale', ylab='log-likelihood')
    dev.copy2eps(file = paste(path,'demOptimiseGp_2_', as.character(counter), '.eps', sep='')) ## Save plot as eps
    system(paste('eps2png ',path,'demOptimiseGp_2_', as.character(counter), '.eps',sep=''))
  }
  ## Convert the .png files to one .gif file using ImageMagick. 
  ## The -delay flag sets the time between showing
  ## the frames, i.e. the speed of the animation.
  system(paste('convert -delay 80 ',path,'demOptimiseGp_1_*.png ', path,'demOptimiseGp_1.gif', sep=''))
  system(paste('convert -delay 80 ',path,'demOptimiseGp_2_*.png ', path,'demOptimiseGp_2.gif', sep=''))
}
