% GENERATEPSREGRESSIONDATA Tries to load a point-set regression sampled data set otherwise generates it.
...
try
  ...
  if noFile
   ...
   [X, y] = psSampleData(trueTheta, kernelType, numIn, numTasks, numDataPerTask);
   [testX, testY] = psSampleData(trueTheta, kernelType, numIn, numTasksTest, numDataPerTaskTest);

   save('psRegressionData.mat', 'trueTheta', 'X', ...
	'y', 'testX', 'testY')
  else
    error(lasterr)
  end
