...
%	Description:
%
%	IVMDOWNDATEM(MODEL, INDEX) removes the given data point from the IVM
%	representations, in particular the matrices M and L. As well as from
%	mu and varSigma. Unfortunately this operation can be numerically
%	unstable: if it is done too many times these represenations must be
%	recomputed (see ivmComputeLandM).
%	 Arguments:
%	  MODEL - the model from which the point is to be removed.
%	  INDEX - the index of the point to remove from the model.
%	ivmUpdateM, rocholhFactorise, rocholFactorise, rocholTransMultiply,
%	rocholForeSub
%	
%
%	See also
%	IVMREMOVEPOINT, IVMEPUPDATEPOINT, IVMCOMPUTELANDM, 
