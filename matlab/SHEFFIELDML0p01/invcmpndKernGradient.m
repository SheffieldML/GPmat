...
%	Description:
%
%	G = INVCMPNDKERNGRADIENT(KERN, X, PARTIAL) computes the gradient of
%	functions with respect to the kernel's parameters theta_1,
%	theta_2... . As well as the kernel structure and the input
%	positions, the user provides a matrix PARTIAL which gives the
%	partial derivatives of the function L with respect to the relevant
%	elements of the kernel matrix, i.e. dL/dK. Then, dL/d theta = trace(
%	dL/dK * dK/d theta) But dK/d theta_i = K ( inv(Ki) dKi/d theta_i
%	inv(Ki) ) K, if theta_i is a parameter of Ki. We augment the partial
%	derivatives matrix with the extra terms above, when we calculate
%	dKi/ dtheta_i, to automatically get the correct expression from each
%	function to xxxKernGradient, where xxx is the type of Ki.
%	 Returns:
...
%
%	G = INVCMPNDKERNGRADIENT(KERN, X1, X2, PARTIAL) computes the
%	gradient of functions with respect to the kernel's parameters
%	theta_1, theta_2... . As well as the kernel structure and the input
%	positions, the user provides a matrix PARTIAL which gives the
%	partial derivatives of the function L with respect to the relevant
%	elements of the kernel matrix, i.e. dL/dK. Then, dL/d theta = trace(
%	dL/dK * dK/d theta) But dK/d theta_i = K ( inv(Ki) dKi/d theta_i
