...
%	Description:
%
%	G = LMCKERNGRADIENT(KERN, X, COVGRAD) computes the gradient of
%	parameters associated to the LMC kernel. As well as the kernel
%	structure and the input positions, the user provides a matrix
%	COVGRAD which gives the partial derivatives of the function with
...
%	   computed.
%	  COVGRAD - matrix of partial derivatives of the function of
%	   interest with respect to the kernel matrix. The argument takes the
%	   form of a square matrix of dimension  numData, where numData is
%	   the number of rows in X.
%
%	G = LMCKERNGRADIENT(KERN, X, X2, COVGRAD) computes the derivatives
...
%	  KERN - the kernel structure for which the gradients are being
%	   computed.
%	  X - the input locations associated with the rows of the kernel
%	   matrix.
