...
%	Description:
%
%	K = LMCKERNCOMPUTE(KERN, X, KERNEL.) computes the kernel matrix for
%	the linear model of coregionalization kernel given inputs associated
%	with X (rows) and X2 (columns).
%	 Returns:
...
%	   the kernel.
%
%	K = LMCKERNCOMPUTE(KERN, X) computes the kernel matrix for the LMC
%	kernel function given X.
%	 Returns:
%	  K - the kernel matrix computed at the given points.
%	 Arguments:
%	  KERN - the kernel structure for which the matrix is computed.
...
%
%	See also
%	LMCKERNPARAMINIT, KERNCOMPUTE, KERNCREATE
