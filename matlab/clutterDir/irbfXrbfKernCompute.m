function K = irbfXrbfKernCompute(irbfKern, rbfKern, t1, t2)

% IRBFXRBFKERNCOMPUTE Compute a cross kernel between the IRBF and RBF kernels.
% FORMAT
% DESC computes cross kernel terms between IRBF and RBF kernels for
...
if rbfKern.variance ~= 1
  warning(['RBF kernel variance = ' num2str(rbfKern.variance) ...
      '. IRBF kernel can only be cross combined with an RBF kernel with variance 1.'])
end

dim1 = size(t1, 1);
dim2 = size(t2, 1);
t1 = t1 - irbfKern.delay;
...
halfSigmaD_i = 0.5*sigma*irbfKern.decay;
...
K = exp(halfSigmaD_i*halfSigmaD_i - irbfKern.decay*diffT + lnPart1);

K = 0.5*sqrt(irbfKern.variance)*K*sqrt(pi)*sigma;
