...
if ~exist('r'),
  for k=1:2,
    for l=1:2,
      tf = tfnames{l};
      r(k).(tf) = drosComputeChipDistanceCurve(chipdistances, drosexp, drosinsitu, {indrank.(tf), disimrank.(tf), mutarank.(tf), corrrank.(tf)}, t(k), tf);
    end
  end
end
...
if ~exist('r_focus'),
  for k=1:2,
    for l=1:2,
      tf = tfnames{l};
      r_focus(k).(tf) = drosComputeChipDistanceCurve(chipdistances, drosexp, drosinsitu, {indrank.(tf), disimrank.(tf), mutarank.(tf), corrrank.(tf)}, t(k), tf, 1);
    end
  end
end
...
set(gca, 'FontSize', FONTSIZE);
axis([10 11 0 1]);
legend('Single-target models', 'Multiple-target models', 'Knock-outs', 'Correlation', 'Random', 'Location', 'North');
