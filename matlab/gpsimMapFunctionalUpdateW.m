      ...
      for l = intPoints
        ...
        for i = 1:length(model.t)
          ...
          if arg1 >= 0 && arg2 >= 0
            for j = 1:model.numGenes
              beta_ij = 1/model.yvar(i+(j-1)*length(model.t));
              temp = temp+beta_ij*model.g_grad(k)*model.g_grad(l)*exp(-model.D(j)*(arg1+arg2))*model.S(j)^2;
            end
          end
        end
        model.W(k, l) = temp;
      ...
      for i=1:length(model.t)
        ...
        if arg >= 0 
          for j=1:model.numGenes
            ...
            temp=temp+factor*model.g_grad2(k)*exp(-model.D(j)*arg)*model.S(j);
          end
        end
      end
      model.W(k, k) = model.W(k, k) + temp*model.step;
    end
    model.updateW = true;
  end
  model.W = 0.5*(model.W + model.W');
  model.W=model.W*model.step^2;
end