...
lnPart1(ind) = log(erf(invSigmaDiffT(ind) - halfSigmaD_i) ...
                   + erf(t2Mat(ind)/sigma + halfSigmaD_i));
lnPart2(ind) = log(erf(t1Mat(ind)/sigma - halfSigmaD_i) ...
                   + erf(halfSigmaD_i));
warning(warnState.state, 'MATLAB:log:logOfZero');

h(ind) = exp(halfSigmaD_i*halfSigmaD_i-D_i*diffT(ind)+lnPart1(ind)...
             - log(D_i + D_j)) ...
         - exp(halfSigmaD_i*halfSigmaD_i-D_i*t1Mat(ind)-D_j*t2Mat(ind) ...
               + lnPart2(ind) ...
               - log(D_i + D_j));
sigma2 = sigma*sigma;
