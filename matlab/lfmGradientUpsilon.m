...
if any(any(ind))
    g(ind) = exp(sigma2*(gamma^2)/4 - gamma*(Tt1(ind)-Tt2(ind)) ...
            + log(sigma2*gamma - 2*(Tt1(ind)-Tt2(ind))) + log(gradThetaGamma+eps)) ...
        - exp(-(Tt1(ind)-Tt2(ind)).*(Tt1(ind)-Tt2(ind))/sigma2 ...
            + log(sigma*gradThetaGamma+eps) + log(eps+1/sqrt(pi) - Z1(ind).*wofz(j*Z1(ind)))) ...
        + exp(-Tt2(ind).*Tt2(ind)/sigma2 - gamma*Tt1(ind) + log(gradThetaGamma+eps) ...
            + log(Tt1(ind).*wofz(j*Z2(ind)+eps) + sigma*(1/sqrt(pi) - Z2(ind).*wofz(j*Z2(ind)))));
end
...
if any(any(ind))
    g(ind) = - exp(-(Tt1(ind)-Tt2(ind)).*(Tt1(ind)-Tt2(ind))/sigma2 ...
            + log(sigma*gradThetaGamma + eps) + log(eps+1/sqrt(pi) + Z1(ind).*wofz(-j*Z1(ind)))) ...
        + exp(-Tt2(ind).*Tt2(ind)/sigma2 - gamma*Tt1(ind) + log(gradThetaGamma + eps) ...
            + log(Tt1(ind).*wofz(j*Z2(ind)) + sigma*(1/sqrt(pi) - Z2(ind).*wofz(j*Z2(ind)))));
end
