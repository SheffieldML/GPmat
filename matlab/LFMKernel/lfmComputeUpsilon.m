...
% values.
%
% COPYRIGHT : David Luengo and Neil D. Lawrence, 2008
%
...
if any(any(ind))
    upsilon(ind) = 2*exp(sigma2*(gamma^2)/4 - gamma*(Tt1(ind)-Tt2(ind))) ...
        - exp(-(Tt1(ind)-Tt2(ind)).*(Tt1(ind)-Tt2(ind))/sigma2 + log(W(j*Z1(ind)))) ...
        - exp(-Tt2(ind).*Tt2(ind)/sigma2 - gamma*Tt1(ind) + log(W(j*Z2(ind))));
end;
