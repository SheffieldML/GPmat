function model = ivmDowndateM(model, ni)
...
if model.Sigma(c).robust
  B = sparse(diag(model.beta(model.I)));
  sqrtB = sqrt(B);
  invSqrtB = sparse(diag(1./sqrt(model.beta(model.I))));
  model.Sigma(c).L = chol(invSqrtB*(model.kern.Kstore(model.I, :) + ...
                       diag(model.beta(model.I)))*invSqrtB)';
  model.Sigma(c).Linv = eye(size(model.Sigma(c).L))/model.Sigma(c).L;
  model.Sigma(c).M = model.Sigma(c).Linv*sqrtB*model.kern.Kstore';
else
  model.Sigma(c).L = chol(model.kern.Kstore(model.I, :) + ...
                       diag(model.beta(model.I)))';
  model.Sigma(c).Linv = eye(size(model.Sigma(c).L))/model.Sigma(c).L;
  model.Sigma(c).M = model.Sigma(c).Linv*model.kern.Kstore';
end
model.J = [model.J ni];
