% DEMUSPSCOMBINE Combine MLP and RBF network predictors.
...
[X, y] = ivmLoadData('usps');

load demUsps1
for i = 1:10
  model1(i) = ivmReconstruct(kernStore{i}, noiseStore{i}, ivmInfoStore{i}, X, y(:, i));
  ll1(i) = ivmLogLikelihood(model1(i));
end
load demUsps2
for i = 1:10
  model2(i) = ivmReconstruct(kernStore{i}, noiseStore{i}, ivmInfoStore{i}, X, y(:, i));
  ll2(i) = ivmLogLikelihood(model2(i));
end
% randn('seed', 1e5)
% rand('seed', 1e5)
