...
for j = 1:3
  varlow{j} = varExp(1:5, 7*(j-1)+1:7*(j-1)+7)';
  varhigh{j} = varExp(6:10, 7*(j-1)+1:7*(j-1)+7)'; 
end
...
%     subplot(3,2,index);
    ...
    bh = [bh plot(predt, predExprs(:,index)-2*real(sqrt(varExprs(:,index))), ...
                  '--')];
    lin1 = plot(times, varlow{j}(:,index), 'rx');
    lin1 = [lin1 plot(times, varhigh{j}(:,index), 'rx')];
    lin1 = errorbar(times, y{j}(:,index), y{j}(:,index)-varlow{j}(:,index), ...
                     varhigh{j}(:,index)- y{j}(:,index), 'rx');
