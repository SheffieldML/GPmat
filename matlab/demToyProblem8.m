% DEMTOYPROBLEM8 Display results from toy problem point at a time.

% GPSIM

load demToyProblem7.mat
...
for i = 1:size(indices, 1)
  ...
  set(gca, 'ylim', [-2 4])
  if bw
    fileName = ['demToyProblem8bw_infered'];
    set(lin, 'color', [0 0 0]);
    set(lin, 'color', [0 0 0]);
    print('-deps', ['../tex/diagrams/' fileName num2str(i)]);
  else
    fileName = ['demToyProblem8_infered'];
    print('-depsc', ['../tex/diagrams/' fileName num2str(i)]);
    pos = get(gcf, 'paperposition');
    origpos = pos;
    pos(3) = pos(3)/2;
