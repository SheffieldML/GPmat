...
% FORMAT
% DESC computes cross kernel terms between LFMA and RBFV kernels for
...
% ARG lfmKern : the kernel structure associated with the LFMA
% kernel.
...
%
% FORMAT
% DESC computes cross kernel terms between LFMA and RBF kernels for
% the multiple output kernel.  This function is employed in the SDLFM kernel
% to compute derivatives with respect to the switching points.
% ARG lfmKern : the kernel structure associated with the LFMA
% kernel.
...
K(:,:) = (sqrt(pi)*sigma*lfmKern.sensitivity/(j*4*lfmKern.mass*omega)) ...
    *(lfmavComputeUpsilonMatrix(gamma2,sigma2,t1, t2, 0) - ...
